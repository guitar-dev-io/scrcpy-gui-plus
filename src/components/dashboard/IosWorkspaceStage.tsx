    ...
    >
      <header className="mb-2 flex min-h-14 shrink-0 flex-wrap items-center gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] px-4 py-2.5">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-0.5 max-w-96 truncate text-[9px] text-[var(--text-subtle)]">
            {[device.udid, `iOS ${device.productVersion}`, device.productType].filter(Boolean).join(' · ')}
          </p>
        </div>
        ...
        <div className="ml-auto flex items-center gap-3 text-[9px] text-[var(--text-subtle)]">
          ...
          >
            <RefreshCw size={11} className={preview.isLoading ? 'animate-spin' : ''} /> Reconnect
          </button>
          <button
            type="button"
            onClick={() => void (preview.isPreviewing ? preview.stop() : preview.start())}
          ...
          <button
            ...
            aria-label={fullscreen ? 'Exit iOS fullscreen' : 'Expand iOS fullscreen'}
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)]">
        <main className="relative flex min-h-0 min-w-0 flex-1 items-center justify-center overflow-auto bg-black/35 p-3">
          <div className={`relative flex max-h-full max-w-full items-center justify-center overflow-hidden rounded-[28px] border-[3px] border-[#3b414d] bg-[#05070b] p-[3px] shadow-[0_18px_42px_rgba(0,0,0,.42)] ${compact ? 'h-[320px]' : 'h-[min(68vh,720px)]'} aspect-[9/19.5]`}>
            {preview.frameSrc ? (
              <img
              ...
              <div className="flex flex-col items-center gap-3 px-6 text-center">
                ...
                <p className="text-[10px] font-semibold text-red-300">Unable to display this iPhone</p>
                <p className="text-[9px] leading-relaxed text-[var(--text-subtle)]">{preview.error}</p>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-3 text-[var(--text-subtle)]">
                <Loader2 size={25} className="animate-spin text-primary" />
                <span className="text-[9px] font-semibold uppercase tracking-widest">Connecting to iPhone</span>
              </div>
            )}
        ...
        {!compact && (
          <aside className="w-60 shrink-0 border-l border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-4">
            ...
            <p className="mt-3 text-[9px] leading-relaxed text-[var(--text-subtle)]">
              This workspace uses the iOS developer screenshot interface. Touch, keyboard, audio, install, shell and file actions are intentionally unavailable.
            </p>
            <dl className="mt-5 space-y-3 border-t border-[var(--border-subtle)] pt-4 text-[9px]">
              <div><dt className="text-[var(--text-subtle)]">Product</dt><dd className="mt-1 text-[var(--text-muted)]">{device.productType || '—'}</dd></div>
              <div><dt className="text-[var(--text-subtle)]">System</dt><dd className="mt-1 text-[var(--text-muted)]">iOS {device.productVersion || '—'}</dd></div>
              <div><dt className="text-[var(--text-subtle)]">Connection</dt><dd className="mt-1 text-[var(--text-muted)]">{device.connectionType || '—'}</dd></div>
              <div><dt className="text-[var(--text-subtle)]">Stream</dt><dd className="mt-1 text-[var(--text-muted)]">PNG · {connected ? `${preview.fps} FPS` : status}</dd></div>
            </dl>
          </aside>
        )}
      </div>
