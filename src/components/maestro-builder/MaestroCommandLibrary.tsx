...
import type { MaestroCommandCategory, MaestroCommandId } from '../../types/maestroBuilder'
import { MAESTRO_COMMON_COMMANDS, searchMaestroCommands } from '../../utils/maestroCommandRegistry'
...
export default function MaestroCommandLibrary({ onAddCommand }: MaestroCommandLibraryProps) {
    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="shrink-0 border-b border-[var(--border-subtle)] p-2">
        <div className="relative">
          <Search size={11} className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-[var(--text-subtle)]" />
          <input
            value={query}
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-2">
        {!query && MAESTRO_COMMON_COMMANDS.length > 0 && (
          <div>
            <p className="mb-1 px-1 text-[8px] font-black uppercase tracking-widest text-[var(--text-subtle)]">Common</p>
        ...
        {results.length === 0 && (
          <p className="px-1 text-[9px] text-[var(--text-subtle)]">No actions match "{query}".</p>
        )}
      </div>
    </div>
  )
