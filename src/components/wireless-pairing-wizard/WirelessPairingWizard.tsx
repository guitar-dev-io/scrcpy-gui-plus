...
type Method = 'pair' | 'ip' | 'scan' | 'recent';
...
interface WirelessPairingWizardProps {
    ...
    discoverConnectAddress: (ip: string, customPath?: string) => Promise<string | null>;
...
export default function WirelessPairingWizard({
    ...
    const handlePair = async () => {
        ...
        report(res, t('pairing.connectedMessage', { address: res.connectedAddress || '' }));
    };
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 sm:p-6">
            ...
            <div className="relative w-full max-w-lg max-h-[90vh] flex flex-col bg-zinc-950/95 border border-zinc-800 rounded-3xl shadow-2xl overflow-hidden backdrop-blur-2xl animate-in zoom-in-95 fade-in duration-200">
                ...
                <div className="px-6 pt-3">
                    <div className="bg-black/40 p-1 rounded-lg grid grid-cols-4 gap-0.5 border border-zinc-800/50">
                        {METHODS.map((m) => (
                            <button
                                key={m.id}
                                onClick={() => {
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4">
                    ...
                    {method === 'pair' && (
                        <div className="space-y-2">
                            ...
                            <button
                                onClick={handlePair}
                                disabled={wizard.busy || !pairAddress.trim() || !pairCode.trim()}
                            ...
                            >
                                {wizard.busy ? <Loader2 size={13} className="animate-spin" /> : <KeyRound size={13} />}
                    ...
                    {method === 'ip' && (
                        <div className="space-y-2">
                            ...
                            <button
                                ...
                                className="w-full py-2.5 rounded-xl bg-primary text-on-primary text-[10px] font-black uppercase tracking-widest hover:brightness-110 transition-all disabled:opacity-30 flex items-center justify-center gap-1.5"
                            >
                                {wizard.busy ? <Loader2 size={13} className="animate-spin" /> : <Wifi size={13} />}
                                {t('pairing.connect')}
                            </button>
                        </div>
                    )}
                    ...
                    {method === 'scan' && (
                        <div className="space-y-2">
                            ...
                            >
                                <RefreshCw size={12} className={wizard.scanning ? 'animate-spin' : ''} />
                            ...
                            {wizard.lanDevices.length === 0 ? (
                                <p className="text-[9px] text-zinc-700 uppercase tracking-widest py-4 text-center">
                                    {wizard.scanning ? t('pairing.scanning') : t('pairing.noLanDevices')}
                                </p>
                            ) : (
                    ...
                    {outcome && (
                        ...
                        >
                            ...
                            <span className={`text-[10px] font-bold ${outcome.success ? 'text-emerald-400' : 'text-red-400'}`}>
                                {outcome.success
                                    ? t('pairing.connectedMessage', { address: outcome.connectedAddress || '' })
                                    : outcome.errorKey
                                      ? t(outcome.errorKey)
                                      : outcome.raw}
                            </span>
                ...
                <div className="px-6 py-3 border-t border-zinc-800/60 flex items-center justify-between">
                    ...
                    >
                        <Zap size={12} className={isAutoConnect ? 'text-primary' : 'text-zinc-600'} />
